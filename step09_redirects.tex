\section{Step 8: Redirect Following}

Many URLs answer with \texttt{301}/\texttt{302}/\texttt{307} and a
\texttt{Location:} header. A usable client should re-issue the request at the
new URL (unless the user disables that). We wrap the fetch path in a loop
controlled by \texttt{follow\_redirects} and \texttt{max\_redirects}.

\subsection{Design Choices}

\begin{itemize}[leftmargin=1.4em]
    \item Keep a mutable \texttt{current\_url} buffer; each redirect overwrites
          it and we \texttt{continue} the loop.
    \item Re-run \texttt{parse\_url}, DNS/proxy connect, TLS, and the HTTP
          exchange for every hop---simpler than special-casing relative
          locations in this minimal tool (absolute \texttt{Location} URLs work
          best).
    \item Cap hops (default~10) to avoid infinite redirect loops.
    \item \texttt{--no-location} sets \texttt{follow\_redirects = false}.
\end{itemize}

\subsection{Detecting Redirects}

After reading the response into \texttt{buf}, a simple approach (used by our
final program) scans for the substring \texttt{Location:}. This is not a full
HTTP parser---it is good enough for a teaching client when the header appears
in the first read chunk.
...
if (follow_redirects && strstr(buf, "Location:") &&
    ...
    if (location[0]) {
        ...
        continue; /* retry outer while with new URL */
    }
}
break; /* done */
\end{lstlisting}

\subsection{Loop Skeleton}
...
while (redirect_count <= max_redirects) {
    ...
    /* connect (+ optional SOCKS5) ... */
    /* optional HTTPS ... */
    /* send request, read body into buf / stdout ... */

    if (/* Location found and following enabled */)
        continue;
    break;
}
\end{lstlisting}

\FullSourceStart{urlgetstep08}
\begin{lstlisting}[style=oldcode,language=C,firstnumber=1]
/* urlget.c -- Step 8: redirect following */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>
...
./urlget http://example.com/          # often 301 -> https
./urlget --no-location http://example.com/
./urlget --max-redirects 2 http://example.com/
\end{lstlisting}
