...
{
    DEBUG_ENTER("(arg=%s)", arg);
    char buf[1024];
    if (strlen(arg) >= sizeof(buf)) { DEBUG_ERROR("URL too long"); return -1; }
    strcpy(buf, arg);
    ...
    if (*hostport == '[') {
        ...
        host[hl] = '\0';
        char *aft = cl + 1;
        if (*aft == ':') u->port = (uint16_t)strtol(aft + 1, NULL, 10);
    } else {
    ...
    DEBUG_TRACE("Parsed: https=%d host=%s port=%u path=%s",
                u->https, u->host, u->port, u->path);
    return 0;
}

/* Minimal SOCKS5 with debug */
static int socks5_connect(int sock, const char *host, uint16_t port)
{
    unsigned char buf[512];
    size_t hlen = strlen(host);
    if (hlen > 255) { DEBUG_ERROR("Hostname too long"); return -1; }

    DEBUG_TRACE("SOCKS5 → Greeting");
    buf[0] = 0x05; buf[1] = 0x01; buf[2] = 0x00;
    if (write(sock, buf, 3) != 3) return -1;

    if (read(sock, buf, 2) != 2 || buf[0] != 0x05 || buf[1] != 0x00) {
        DEBUG_ERROR("SOCKS5 greeting rejected");
        return -1;
    }

    DEBUG_TRACE("SOCKS5 → Connect request to %s:%u", host, port);
    ...
    if (read(sock, buf, 4) != 4 || buf[1] != 0x00) {
        DEBUG_ERROR("SOCKS5 connect failed (REP=0x%02x)", buf[1]);
        return -1;
    }

    DEBUG_TRACE("SOCKS5 handshake successful");
    return 0;
}
...
/* RFC 4648 Base64 encoder for HTTP Basic auth (-u USER:PASS). */
...
int main(int argc, char *argv[])
{
    ...
    const char *user_agent = "Mozilla/5.0 (X11; Linux x86_64; rv:128.0) Gecko/20100101 Firefox/128.0";
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-A") == 0 || strcmp(argv[i], "--user-agent") == 0) {
        ...
        else if (strcmp(argv[i], "-T") == 0 || strcmp(argv[i], "--tor") == 0) {
            use_tor = true;
            if (i + 1 < argc && argv[i + 1][0] != '-' && strchr(argv[i + 1], ':')) {
                tor_proxy = argv[i + 1];
                i++;
            }
        }
            ...
            if (i + 1 < argc) {
                strncat(extra_headers, argv[++i], sizeof(extra_headers) - strlen(extra_headers) - 3);
            ...
            if (i + 1 < argc) {
                ...
                if (val[0] == '@') {
                    ...
                    if (f) { fread(post_data, 1, sizeof(post_data)-1, f); fclose(f); }
                } else {
            ...
            if (i + 1 < argc) strncpy(output_file, argv[++i], sizeof(output_file)-1);
            else usage(argv[0]);
        }
        ...
        else if (strcmp(argv[i], "--max-redirects") == 0 && i + 1 < argc) {
    ...
    while (redirect_count <= max_redirects) {
        ...
        if (parse_url(current_url, &u) != 0) {
            DEBUG_ERROR("parse_url failed");
            return EXIT_FAILURE;
        }

        DEBUG_TRACE("Using Tor: %s | Redirects: %d/%d", use_tor ? "yes" : "no", redirect_count, max_redirects);
        ...
        DEBUG_TRACE("Connected to %s:%u%s", connect_host, connect_port, use_tor ? " (Tor)" : "");
        ...
        const char *method = head_request ? "HEAD" : (post_data[0] ? "POST" : "GET");
        ...
        int len = snprintf(req, sizeof(req),
            ...
            "Accept: */*\r\n"
            "Connection: close\r\n",
            method, u.path, u.host, user_agent, extra_headers, auth_header);
        ...
        DEBUG_TRACE("Request sent (%s)", method);

        char buf[8192];
        ssize_t n;
        while ((n = u.https ? SSL_read(ssl, buf, sizeof(buf)) : read(s, buf, sizeof(buf))) > 0) {
            fwrite(buf, 1, n, out);
        }
        ...
        if (follow_redirects && strstr(buf, "Location:") && redirect_count < max_redirects) {
            ...
            if (location[0]) {
                redirect_count++;
                DEBUG_TRACE("Following redirect %d/%d → %s", redirect_count, max_redirects, location);
