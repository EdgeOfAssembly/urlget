\section{Step 10: Final Polish and Release Build}

The functional client is complete. This step adds structured debug tracing
(compile-time removable), feature-test macros for POSIX APIs, and presents the
full production-shaped \texttt{urlget.c}.

\subsection{Debug Infrastructure}

The real program includes a small header \texttt{debug.h} controlled by:

\begin{itemize}[leftmargin=1.4em]
    \item \texttt{DEBUG\_LEVEL} --- verbosity threshold
    \item \texttt{DEBUG\_COLORS} / \texttt{DEBUG\_TIMESTAMPS} --- presentation
    \item \texttt{-DNDEBUG} --- strips all debug calls for release builds
\end{itemize}

Typical macros used in the final source:

\begin{lstlisting}[style=snippet,language=C]
DEBUG_ENTER();
DEBUG_ENTER("(arg=%s)", arg);
DEBUG_TRACE("Connected to %s:%u", host, port);
DEBUG_ERROR("parse_url failed");
\end{lstlisting}

If you do not have \texttt{debug.h}, a minimal stub is enough to compile:

\begin{lstlisting}[style=snippet,language=C]
/* debug.h -- minimal stub */
#ifndef DEBUG_H
#define DEBUG_H
#ifdef NDEBUG
...
# Development (debug traces on)
gcc -Wall -Wextra -O0 -g -o urlget urlget.c -lssl -lcrypto

# Release (no debug noise)
gcc -Wall -Wextra -O2 -DNDEBUG -o urlget urlget.c -lssl -lcrypto
\end{lstlisting}

\subsection{Feature Map (where everything lives)}

\begin{center}
\begin{tabular}{@{}ll@{}}
\hline
\textbf{Concern} & \textbf{Location in final source} \\
\hline
URL $\rightarrow$ host/port/path & \texttt{url\_t}, \texttt{parse\_url} \\
CLI options & \texttt{usage}, \texttt{main} arg loop \\
Plain TCP & \texttt{getaddrinfo} / \texttt{socket} / \texttt{connect} \\
TLS & \texttt{https\_setup}, \texttt{SSL\_read}/\texttt{write} \\
Tor & \texttt{socks5\_connect}, \texttt{-T} path \\
Redirects & outer \texttt{while} + \texttt{Location:} scan \\
POST / HEAD / headers / \texttt{-o} & request builder + \texttt{out} FILE \\
Basic auth (\texttt{-u}) & \texttt{base64\_encode} + \texttt{Authorization} header \\
\hline
\end{tabular}
\end{center}

\subsection{Final Complete Source}

The listing below is the complete \texttt{urlget.c} as developed in this
tutorial (debug calls included). Study it as one coherent program---every
piece was introduced with a reason in an earlier step.

\begin{lstlisting}[style=fullcode,language=C]
...
int main(int argc, char *argv[])
{
    ...
    while (redirect_count <= max_redirects) {
        ...
        DEBUG_TRACE("Using Tor: %s | Redirects: %d/%d",
                    use_tor ? "yes" : "no",
        ...
        DEBUG_TRACE("Connected to %s:%u%s", connect_host, connect_port,
                    use_tor ? " (Tor)" : "");
        ...
        if (follow_redirects && strstr(buf, "Location:") &&
            ...
            if (location[0]) {
                redirect_count++;
                DEBUG_TRACE("Following redirect %d/%d -> %s",
                            redirect_count, max_redirects, location);
...
You now have a single-file HTTP(S) client that:

\begin{itemize}[leftmargin=1.4em]
    \item Parses URLs and options explicitly
    \item Speaks TCP, optional TLS, and optional SOCKS5
    \item Follows redirects with a safety limit
    \item Supports the common request customizations of a mini-\texttt{curl},
          including curl-style \texttt{-u USER:PASS} with built-in Base64
\end{itemize}

By \texttt{cloc}, the finished \texttt{urlget.c} is about \textbf{348 lines of
code} (plus a handful of blanks and comments). Not bad for a pocket-sized
mini-\texttt{curl}.

Natural next exercises: verify TLS certificates properly, parse HTTP status
lines and headers fully, and support chunked transfer encoding.
